...
function initBoard() {
    ...
    moves = 0;            // Reset moves
    timer = 0;            // Reset timer
    toggledCells.clear(); // Clear toggled cells set
...
function toggleLights(row, col) {
    ...
    if (lastCell === cellKey) {
        // If the same cell is clicked consecutively
        toggledCells.delete(cellKey);
        moves--; // Decrement moves
        lastCell = null; // Reset lastCell to ensure next click increments
    } else {
        // If a different cell is clicked
        toggledCells.add(cellKey); // Add to toggled set
        moves++; // Increment moves
        lastCell = cellKey; // Update last clicked cell
    }
